...
__h: dict[str, float] = {'=': 0, '0': 0.15814538269065384, '5->6': 0.3086052044945551, '8->9': 0.32088883648881406, '0->8': 0.3495508187313481, '9->1': 0.35775317139557467, 'X7': 0.41626082849473467, '1->5': 0.4338406619231052, '1': 0.45609453141903045, '2->4': 0.4749963088368894, 'sq': 0.47962756433130904, '4->7': 0.5141588452568465, 'X1': 0.5166560740689605, '6->2': 0.5347275076375902, 'X9': 0.5718535376040897, '*': 0.5792247855829127, '10': 0.5913718846194719, '6X': 0.6334435639230976, 'X2': 0.6349205354407589, '7->3': 0.6366456684326074, '4X': 0.6864526870725012, 'X4': 0.6908280317772608, '3X': 0.721412520940575, '5X': 0.7339595009561901, '8X': 0.747595334523549, '2': 0.7479166944592697, '6': 0.7759437542357516, '4': 0.7812672095691643, '5': 0.8423289904397462, '3->0': 0.8452956735676734, 'X++': 0.8654186842871379, '9': 0.8799291472058557, '8': 0.9315283519239104, '3': 0.9590864288247053, '7': 1.0059478347845556, '+': 1.0803406986272548, 'X-10': 1.121841352851098, 'coins': 1.1440189961830383, 'X+10': 1.1802455962125433, 'X->25': 1.212591160620958, '-': 1.271571989304066, 'primes': 1.3041465160091292, 'X--': 1.406763938975868, '%': 1.45528232483905, 'X->0': 1.6233866742100986, 'cut': 1.8032331706128801, 'last': 1.836336296025939, 'swap': 1.8576454969390899, 'reverse': 1.937386749521466, '/': 2.1133213684603023, 'sqr': 2.8969624411299044, 'near': 4.409564378136184}
for name in __h:
    button_costs_default[name] = __h[name]
...
def check_button_sequence(button_sequence:list[str], buttons_availible:list[str], number_current:int, number_target:int, coins:int=0, last:int|None=None, cost_maximum:float|None=None) -> tuple[float, Literal["SOLVED"] | Literal["NOT SOLVED"] | Literal["INVALID"]]:
    ...
    for action in button_sequence:
        ...
        if eval_equals or action == "=":
            ...
            if last_operation == '+': number_current += int(number_input) # type: ignore
            elif last_operation == '-': number_current = max(number_current - int(number_input), 0) # type: ignore # the game does not allow you to be under 0, I learned that the hard way
            elif last_operation == '*': number_current *= int(number_input) # type: ignore
            elif last_operation == '/':
