...
from sympy import isprime, nextprime, prevprime  # type: ignore

digits: set[str] = {str(i) for i in range(100)}
operations_simple: set[str] = {
    "sq",
    "sqrt",
    "swap",
    "primes",
...
operations_replace: set[str] = {
    f"{a}->{b}" for a in range(10) for b in range(10) if a != b
}
operations_replace_in_game: list[str] = [
    "2->4",
    "4->7",
    "7->3",
    "3->0",
...
buttons_implemented: set[str] = (
    digits
    | operations_simple
    | operations_with_argument
    | operations_replace
    | operations_append
    | operations_prepend
    | {"="}
)
buttons_in_game: set[str] = (
    digits
    | operations_simple
    | operations_with_argument
    | set(operations_replace_in_game)
    | operations_append_in_game
    | operations_prepend_in_game
    | {"="}
)
...
def _find_nearest_prime(number_current: int) -> int:
    ...
    if not isinstance(lower, int) or not isinstance(upper, int):
        raise TypeError(
            "'prevprime()' or 'nextprime()' did not return an int. this is a reminder that you should not copy code from chargpt."
        )
...
def _calculate_cost(buttons_availible: list[str], action: str) -> float:
    ...
    cost = (
        _cost_multiplier_by_ammount(buttons_availible.count(action))
        * button_costs_default[action]
    )
    return max(cost, 0)


def check_button_sequence(
    button_sequence: list[str],
    buttons_availible: list[str],
    number_current: int,
    number_target: int,
    coins: int = 0,
    last: int | None = None,
    cost_maximum: float | None = None,
) -> tuple[float, Literal["SOLVED"] | Literal["NOT SOLVED"] | Literal["INVALID"]]:
    """
    ...
    if len(button_sequence) == 0:
        if number_current == number_target:
            return (cost, "SOLVED")
        else:
            return (cost, "NOT SOLVED")
    if button_sequence[-1] != "=":
        button_sequence.append("=")
    if buttons_implemented | set(button_sequence) != buttons_implemented:
        return (
            cost,
            "INVALID",
        )  # <- invalid, because button_sequence contains invalid buttons
    ...
    for action in button_sequence:
        ...
        if cost_maximum is not None and cost >= cost_maximum:
            return (cost, "NOT SOLVED")  # costs too much

        if action in digits:
            if last_operation not in operations_with_argument:
                return (
                    cost,
                    "INVALID",
                )  # <- invalid input ("5 sq 2" does not make sense)
            if number_input == "" and action == "0":
                return (
                    cost,
                    "INVALID",
                )  # <- leading zeros are a waste of button inputs; can be generated without the leading zero
            number_input += action
        ...
        if action in (buttons_implemented ^ digits):
            if last_operation in operations_with_argument and number_input == "":
                return (
                    (cost, "INVALID") if action != "=" else (cost, "NOT SOLVED")
                )  # <- invalid input ("x + /" does not make sense) (but we do not want to add "+" to the invalid_trees either)
            cost += _calculate_cost(buttons_availible=buttons_availible, action=action)
            if action != "=":
                buttons_availible.remove(action)
            if last_operation != "=":
        ...
        if eval_equals or action == "=":
            if last_operation == "=" and action == "=":
                continue
            if last_operation in operations_with_argument and number_input == "":
                return (
                    (cost, "INVALID") if action != "=" else (cost, "NOT SOLVED")
                )  # <- invalid input ("x + /" does not make sense) (but we do not want to add "ad+d" to the invalid_trees either)

            if last_operation == "+":
                number_current += int(number_input)  # type: ignore
            elif last_operation == "-":
                number_current = max(number_current - int(number_input), 0)  # type: ignore # the game does not allow you to be under 0, I learned that the hard way
            elif last_operation == "*":
                number_current *= int(number_input)  # type: ignore
            elif last_operation == "/":
                if int(number_input) == 0:
                    return (cost, "INVALID")  # <- invalid input (a // 0 is not defined)
                ...
                if int(number_input) <= 0:
                    return (
                        cost,
                        "INVALID",
                    )  # <- invalid input (a mod 0 is not defined)
                number_current = number_current % int(number_input)  # type: ignore

            elif last_operation == "sq":
                number_current = number_current * number_current
            elif last_operation == "sqrt":
                if int(number_current) < 0:
                    return (
                        cost,
                        "INVALID",
                    )  # <- invalid input (squareroot of negatives is not allowed)
                number_current = round(number_current**0.5)
            elif last_operation == "swap":
                number_current, number_target = number_target, number_current
            elif last_operation == "primes":
                number_current = _find_nearest_prime(number_current=number_current)
            elif last_operation == "X++":
                number_current += 1
            elif last_operation == "X--":
                number_current = max(number_current - 1, 0)
                ...
                if len(str(number_target)) < 2:
                    return (
                        cost,
                        "INVALID",
                    )  # <- the game does not cut the first digit, if it is the only one
                number_target = int(str(number_target)[1:])
            elif last_operation == "X+10":
                number_current += 10
            elif last_operation == "X-10":
                number_current -= 10
            elif last_operation == "coins":
                number_current += coins
            elif last_operation == "last":
                if last is None:
                    return (
                        cost,
                        "INVALID",
                    )  # <- can not use "last" if there is no number provided
                number_current = last
            elif last_operation == "100-X":
                number_current = max(0, 100 - number_current)
            elif last_operation in operations_replace:
                if len(last_operation) != 4:
                    raise ValueError(
                        "'last_operation' can not be parsed since it is not of length 4. How did that even happen?"
                    )
            ...
            if number_current != number_last:
                ...
                return (
                    cost,
                    "INVALID",
                )  # <- means whatever steps where taken did only increase cost but did not change anything

            number_input = ""
            last_operation = (
                "=" if last_operation_buffer == "" else last_operation_buffer
            )
            continue
...
def get_local_buttons_availible(
    buttons_availible: list[str], buttons_used: list[str]
) -> list[str]:
    ...
    try:
        while (iterations < max_iterations) and branches != []:
            ...
            if debug and (iterations % 1000) == 0:
                print(
                    f"\riterations: {iterations//1000:>0{iterations_max_len-3}}k | solutions: {solutions_ammount:>0{solutions_ammount_max_len}} | len(branches): {len(branches)//1000:>06}k",
                    end="",
                )
            ...
            cost, solved = check_button_sequence(
                button_sequence=button_sequence.copy(),
                buttons_availible=buttons.copy(),
            ...
            if solved == "INVALID":
                ...
                if solved != "NOT SOLVED":
                    raise ValueError(
                        f"How does the button sequence not resolve to either 'SOLVED', 'NOT SOLVED' or 'INVALID' but instead '{solved}'"
                    )

            branches_new: list[tuple[float, list[str]]] = []
            local_buttons_availible = get_local_buttons_availible(
                buttons_availible=buttons, buttons_used=button_sequence
            )
            for button in set(local_buttons_availible):
                cost_new = cost + _calculate_cost(
                    buttons_availible=local_buttons_availible, action=button
                )
...
def main():
    buttons: list[str] = [str(i) for i in range(10)] * 2 + [
        "+",
        "-",
        "*",
        "/",
